...
def create_clinician_action(action: schemas.ClinicianActionCreate, db: Session = Depends(get_db)):
    proposal = db.query(models.AgentProposal).filter(models.AgentProposal.id == action.proposal_id).first()
    if not proposal:
        raise HTTPException(status_code=404, detail="Proposal not found")
    if proposal.status != "pending":
        raise HTTPException(status_code=400, detail=f"Proposal already resolved with status '{proposal.status}'")
